...
import './things-player-grid'

class ThingsPlayerCarousel extends LitElement {
  static get is() {
    return 'things-player-carousel'
  }
  ...
  build() {
    ...
    var panel = Array.from(this.carousel.querySelectorAll('things-player-grid')).pop()
    ...
    while (page) {
      if (!(i++ % (rows * columns))) {
        panel = document.createElement('things-player-grid')
        panel.rows = rows
  ...
  start() {
    var panels = this.carousel.querySelectorAll('things-player-grid')
